...
impl ApiSession {
    pub async fn add_credential(&mut self, credential: NewCredential) -> Result<u16, ApiError> {
        ...
        let service = cipher.encrypt(credential.service.as_bytes()).map_err(|error| {
            log::error!("Error encrypting service {:?}", error);
            ApiError::Encryption
        })?;
        let username = cipher.encrypt(credential.username.as_bytes()).map_err(|error| {
            log::error!("Error encrypting username {:?}", error);
            ApiError::Encryption
        })?;
        let password = cipher.encrypt(credential.password.as_bytes()).map_err(|error| {
            log::error!("Error encrypting password {:?}", error);
            ApiError::Encryption
        })?;
        let notes: Option<Vec<Vec<u8>>> = if let Some(notes) = credential.notes {
            Some(notes.iter().map(|note| -> Result<Vec<u8>, ApiError> {
                Ok(cipher.encrypt(note.as_bytes()).map_err(|error| {

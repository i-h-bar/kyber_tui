    ...
    PW: PWStore + Send + Sync,
{
    pub async fn add_credential(
        ...
        let notes: Option<Vec<Note>> = match new_credential.notes {
            Some(notes) => Some(
                notes
                    .iter()
                    .filter_map(|note| {
        ...
        let response = NewCredentialResponse {
            added: 1,
        };

        let response = GenericResponse {
            body: session.server_key_pair.encrypt(&response, &session.client_public_key).unwrap(),
            token: request.token,
        };

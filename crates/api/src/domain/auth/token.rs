    ...
    PW: PWStore + Send + Sync,
{
    ...
    pub async fn verify_auth_token(
        &self,
        request: &GenericRequest,
    ) -> Result<(Session, AuthToken), DomainError> {
        let session = self.cache.load_session(&request.session_id).await?;
        let token = check_auth_token(&request.token, &session)?;

        Ok((session, token))
    }
}

fn check_auth_token(token: &EncryptedMessage, session: &Session) -> Result<AuthToken, DomainError> {
    ...
    let Some(session_user_id) = session.user_id else {
        return Err(DomainError::Permission(
            "Session user id not set".to_string(),
        ));
    };

    if token.user_id != session_user_id {
        log::warn!(
        ...
        return Err(DomainError::Permission(
            "Session & Token user id mismatch".to_string(),
        ));
    }
